use rabbitmq_stream_client::error::{StreamCreateError, ProducerPublishError};
use rabbitmq_stream_client::types::{ByteCapacity, Message, ResponseCode};
use rabbitmq_stream_client::{ConfirmationStatus, NoDedup, OnClosed, Producer, RabbitMQStreamResult};
...
impl OnClosed for MyHAProducer {
    async fn on_closed(&self) {
        ...
        let new_producer = self.environment
            .producer()
            .build(&self.stream).await
            .unwrap();
...
impl MyHAProducer {
    ...
    async fn send_with_confirm(&self, message: Message) -> Result<ConfirmationStatus, ProducerPublishError> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    producer.send_with_confirm(Message::builder().body("Hello, world!").build()).await?;
